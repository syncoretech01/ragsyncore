...
@app.route("/voiceflow/query", methods=["POST"])
def voiceflow_query():
    """Simplified endpoint specifically for Voiceflow integration"""
    data = request.get_json()
    question = data.get("question", "")
    ...
    if not question:
        return jsonify({
            ...
            "answer": "I need a question to help you. Please ask me something about Syncore Tech."
        }), 200
    ...
    try:
        ...
        if results.get('matches') and len(results['matches']) > 0:
            ...
            if context_texts:
                ...
                prompt = f"""You are a helpful assistant for Syncore Tech. Based on the following information, provide a clear and conversational answer to the user's question. Keep the answer concise but informative.
...
- Include specific details from the information provided
- If the information doesn't fully answer the question, acknowledge what you can answer
- End with asking if they need more specific information"""
                ...
                return jsonify({
                    ...
                    "sources": [match['metadata'].get('source', 'Unknown') for match in results['matches'][:3]]
                })
            else:
                return jsonify({
                    "success": True,
                    "answer": "I don't have enough relevant information to answer that question. Could you please rephrase or ask something else about Syncore Tech's services?",
                    "confidence": 0
                })
        else:
            return jsonify({
                "success": True,
                "answer": "I couldn't find specific information about that. Please ask me about Syncore Tech's services like web development, SEO, digital marketing, or e-commerce solutions.",
                "confidence": 0
            })
        ...
        return jsonify({
            "success": False,
            "message": str(e),
            "answer": "I'm having trouble accessing the information right now. Please try again later."
        }), 200

@app.route("/voiceflow/categories", methods=["GET"])
def get_categories():
    """Return available categories for Voiceflow to use in conversation flows"""
    categories = {
        "services": [
            "Web Development",
